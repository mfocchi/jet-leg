    ...
    if max_wrench is not None:
        ...
        if direction_of_max_wrench is not None:
            ...
            for i in range(len(t)):
                points_of_line[:,i] = max_wrench + direction_of_max_wrench *t[i]
            ax.scatter(points_of_line[0,:], points_of_line[1,:], points_of_line[2,:],  color='g')
    if static_wrench is not None:
        ax.scatter(static_wrench[0], static_wrench[1], static_wrench[2], marker='o', color='g', s=1000)
...
#W_rope_force_sx = np.hstack((np.zeros((3,1)), W_rope_axis_sx*max_rope_force))
#W_rope_force_dx = np.hstack((np.zeros((3,1)), W_rope_axis_dx*max_rope_force))
...
res = computeMargin(FWP, np.array([0.,  0,  1. ,0,0,0]), offset = w_gi)
...
# Debug
direction_of_max_wrench = np.array([0.,  1,  1.])
res = computeMargin(FWP, direction_v=direction_of_max_wrench , offset = np.array([400,0,0]), type_of_margin='3D')
plot_FWP(FWP, "FWP", max_wrench=res.x, direction_of_max_wrench=direction_of_max_wrench)