...
LC_foot = np.array([0.0, 0.37, 0.02])
RC_foot = np.array([0.0, -0.28, 0.02])

contactsWF = np.vstack((LF_foot, RF_foot, LH_foot, RH_foot, LC_foot, RC_foot))
numberOfLegs = contactsWF.shape[0]
...
n5 = np.transpose(np.transpose(math.rpyToRot(0.0,0.0,0.0)).dot(axisZ))  # LC
n6 = np.transpose(np.transpose(math.rpyToRot(0.0,0.0,0.0)).dot(axisZ))  # RC
normals = np.vstack([n1, n2, n3, n4, n5, n6])

plane_normal = [0.01, 0.01, 0.99]
...
params.setNoOfLegs(numberOfLegs)
